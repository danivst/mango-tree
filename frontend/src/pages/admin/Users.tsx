...
const Users = () => {
  ...
  // Compute special categories for preview
  ...
  // Language for category names in preview
  const categoryDisplayLanguage = selectedUser?.language === "bg" ? "bg" : language;

  const getCategoryDisplayName = (categoryName: string, lang: string = categoryDisplayLanguage) => {
    const translated = getTranslation(lang as any, categoryName.toLowerCase());
    if (translated && translated !== categoryName.toLowerCase()) return translated;
    return categoryName.charAt(0).toUpperCase() + categoryName.slice(1);
  };

  // Filter users by search (username or email)
  ...
  const handleDeleteBack = () => { setDeleteStep("warning"); setDeleteReason(""); };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setDeleteReason("");
      await fetchUsers();
      try { await fetchReports(); await fetchFlaggedContent(); } catch (err) { console.error(err); }
    } catch (error: any) {
  ...
  const handleBanBack = () => { setBanUnbanStep("warning"); setBanReason(""); };
  ...
  // View Profile column (non-sortable, part of columns)
  ...
  // Prepend viewProfile column to columns? Actually we want columns array to include it as 4th column before actions.
          ...
          <div className="modal modal-danger">
            {deleteStep === "warning" && (
              <>
                ...
                <div className="modal-actions">
                  <button className="btn-secondary" onClick={() => { setDeleteStep(null); setDeleteUserId(null); }}>{t("close")}</button>
                  <button className="btn-primary" onClick={handleDeleteContinue}>{t("continue")}</button>
                </div>
              </>
            )}
            {deleteStep === "reason" && (
              <>
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleDeleteTerminate(); }}>
                  <div className="form-group">
                    ...
                    <textarea className="form-textarea" value={deleteReason} onChange={(e) => setDeleteReason(e.target.value)} required rows={4} placeholder={t("adminReasonForDeletionPlaceholder")} />
                  </div>
                  <div className="modal-actions">
                    <button type="button" className="btn-secondary" onClick={handleDeleteBack}>{t("goBack")}</button>
                    <button type="submit" className="btn-primary">{t("terminateAccount")}</button>
                  </div>
                </form>
              </>
            )}
            {deleteStep === "confirm" && (
              <>
                ...
                <p className="modal-text">{t("adminConfirmDeletionText").replace("{username}", userToDelete.username)}</p>
                <div className="modal-actions">
                  <button className="btn-secondary" onClick={() => setDeleteStep("reason")}>{t("no")}</button>
                  <button className="btn-danger" onClick={handleDeleteConfirm}>{t("yes")}</button>
                </div>
              </>
            )}
          </div>
          ...
          <div className="modal modal-danger">
            {banUnbanStep === "warning" && (
              <>
                ...
                <p className="modal-text">{t("banUserWarning").replace("{username}", userToBan.username)}</p>
                <div className="modal-actions">
                  <button className="btn-secondary" onClick={() => { setBanUnbanStep(null); setBanUserId(null); }}>{t("close")}</button>
                  <button className="btn-primary" onClick={handleBanContinue}>{t("continue")}</button>
                </div>
              </>
            )}
            {banUnbanStep === "reason" && (
              <>
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleBanTerminate(); }}>
                  <div className="form-group">
                    <label className="form-label">{t("reasonForBan")}</label>
                    <textarea className="form-textarea" value={banReason} onChange={(e) => setBanReason(e.target.value)} required rows={4} placeholder={t("reasonForBanPlaceholder")} />
                  </div>
                  <div className="modal-actions">
                    <button type="button" className="btn-secondary" onClick={handleBanBack}>{t("goBack")}</button>
                    <button type="submit" className="btn-danger">{t("banUser")}</button>
                  </div>
                </form>
              </>
            )}
            {banUnbanStep === "confirm" && (
              <>
                ...
                <p className="modal-text">{t("confirmBanText").replace("{username}", userToBan.username)}</p>
                <div className="modal-actions">
                  <button className="btn-secondary" onClick={() => setBanUnbanStep("reason")}>{t("goBack")}</button>
                  <button className="btn-danger" onClick={handleBanConfirm}>{t("banUser")}</button>
                </div>
              </>
            )}
          </div>
      ...
      {banUnbanStep === "unban_confirm" && banUserId && userToUnban && (
        <div className="modal-overlay">
          <div className="modal modal-danger">
            ...
            <p className="modal-text">{t("unbanUserConfirm").replace("{username}", userToUnban.username)}</p>
            <div className="modal-actions">
              <button className="btn-secondary" onClick={() => { setBanUnbanStep(null); setBanUserId(null); }}>{t("cancel")}</button>
              <button className="btn-primary" onClick={handleUnbanConfirm}>{t("unban")}</button>
            </div>
          </div>
        </div>
      )}
            ...
            {previewLoading ? (
              ...
              <>
                <div className="modal-close-container">
                  <button onClick={() => setSelectedUser(null)} className="btn-close">&times;</button>
                </div>
                <div className="preview-header">
                  <div className="preview-avatar-container">
                    <div className="preview-avatar">
                      {selectedUser.profileImage ? (
                        <img src={selectedUser.profileImage} alt={selectedUser.username} className="preview-avatar-image" />
                      ) : (
                        <div className="preview-avatar-placeholder">{selectedUser.username.charAt(0).toUpperCase()}</div>
                      )}
                    </div>
                  </div>
                  <div className="preview-info">
                    <h1 className="preview-username">@{selectedUser.username}</h1>
                    <p className="preview-member-since">{t("memberSince")}: {new Date(selectedUser.createdAt).toLocaleDateString(language === "bg" ? "bg-BG" : "en-US", { year: "numeric", month: "short", day: "numeric" })}</p>
                    <div className="preview-stats">
                      ...
                      <div className="preview-stat-item">
                        <div className="preview-stat-value">{selectedUser.followers?.length || 0}</div>
                        <div className="preview-stat-label">{t("followers")}</div>
                      </div>
                      <div className="preview-stat-item">
                        <div className="preview-stat-value">{selectedUser.following?.length || 0}</div>
                ...
                {userCategories.length > 0 && (
                  <div className="preview-category-tabs">
                    <button onClick={() => setSelectedUserCategoryId(null)} className={`preview-category-tab ${selectedUserCategoryId === null ? 'active' : ''}`}>{t("all")}</button>
                    {specialCategories.filter((category) => category._id).map((category) => (
                      <button key={category._id} onClick={() => setSelectedUserCategoryId(category._id)} className={`preview-category-tab ${selectedUserCategoryId === category._id ? 'active' : ''}`}>{getCategoryDisplayName(category.name)}</button>
                    ))}
                  </div>
                )}
                {(() => {
                  const filteredPosts = userPosts.filter((post) => !selectedUserCategoryId || (post.category && post.category._id === selectedUserCategoryId));
                  if (filteredPosts.length === 0) {
                    return (
                      <div className="empty-state">
                        <span className="material-icons">article</span>
                        <h3>{selectedUserCategoryId ? t("noPostsFound") : t("noPostsAvailable")}</h3>
                      </div>
                    );
                  }
                  return (
                        ...
                        <div key={post._id} className="card preview-post-card">
                          {post.image && post.image.length > 0 && (
                            <div className="preview-post-image-container">
                              <img src={post.image[0]} alt={post.title} className="preview-post-card-image" />
                            </div>
                          )}
                          ...
                          <div className="preview-post-meta">
                            ...
                            <span>{new Date(post.createdAt).toLocaleDateString(language === "bg" ? "bg-BG" : "en-US", { year: "numeric", month: "short", day: "numeric" })}</span>
                          </div>
                          <div className="preview-post-likes"><span className="material-icons">favorite</span>{post.likes?.length || 0}</div>
                        </div>
                      ))}
                    </div>
                  );
