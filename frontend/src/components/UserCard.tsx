 ...
 *   onToggleFollow={handleToggleFollow}
 *   additionalActions={
 *     <Button variant="danger" onClick={() => handleBan(user._id)}>Ban</Button>
 *   }
 * />
 */
...
const UserCard: React.FC<UserCardProps> = ({
  ...
  return (
    <div className={cardClasses} onClick={handleCardClick} role={onUserClick ? "button" : undefined}>
      {profileImage ? (
        <img
          src={profileImage}
          alt={username}
      ...
      <div className="user-card-info">
        ...
        <p className="user-card-meta">{t("memberSince")}: {formatDate(createdAt)}</p>
      </div>
      <div className="user-card-actions">
        {showActions && (
          <button
            className={`btn-secondary btn-sm icon-btn ${localIsFollowing ? "btn-following" : "btn-follow"}`}
            onClick={handleFollowClick}
            disabled={isLoading}
          >
            <span className="material-icons text-lg">
              {localIsFollowing ? "person_remove" : "person_add"}
            </span>
          </button>
        )}
        {additionalActions}
